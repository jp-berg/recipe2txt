...
Since most parameters provided by the command line rarely change between program uses
it makes sense to provide a
config-file to the user, so that tedious retyping of favored options can be avoided.
And since most parameters a
program needs to derive either a CLI-option or a config option are the same,
there should be a central place to
configure both.
...
class ArgKey(StrEnum):
    """Enum describing all :py:meth:`argparse.ArgumentParser.add_to_parser
...
class BasicOption:
    ...
    def toml_valid(self, value: Any) -> bool:
        """Decides whether the TOML-representation for this option returns a valid
        value"""
        return bool(value)
    ...
    def from_toml(self, toml: dict[str, Any]) -> bool:
        ...
        The value is checked for validity. If its valid it replaces the current
        default-value
        Args:
            toml = A dictionary containing TOML-key-value-pairs (e.g. generated by
            `:py:meth:tomllib.load`)

        Returns:
            True if  a valid value could be extracted from the TOML-file,
            False otherwise
        """
...
class ChoiceOption(BasicOption, Generic[T]):
    """
    Option-Subclass representing the corresponding
    :py:meth:`argparse.ArgumentParser.add_argument`-option, with the
...
class TypeOption(BasicOption):
    ...
    Converts input-strings to the given type instead just parsing everything as a
    string.
    Since the value is being parsed from TOML it should be representable as such (
    i.e. probably only 'int' and 'float'
    make sense as types).
    """
...
class NArgOption(BasicOption):
    ...
    It differs from the method by providing an empty list instead of failing if the
...
class ArgConfig:
    ...
    Checks if there is a config-file already available. If there is such a file,
    every option that is added tries to
    retrieve a default-value from it, otherwise the option will fall back onto the
    given default. If no config-file is
    available, a new one will be created and every option will write a
    TOML-key-value-pair-representation of itself
    ...
    def add_arg(
        ...
        Args:
            name (): The name of the option. Will set an argparse-flag '--name' and
            will be the key in the
                TOML-key-value-pair. Cannot start with an '-'.
            help_str (): Description of the option. Will be the help-text for the
            argparse-flag and a comment just above
                the TOML-key-value-pair corresponding to this option.
            default (): The value that should be used if no other value is provided
            via CLI or TOML. Will be used as the
                initial value in the TOML-key-value-pair.
            short (): Whether there should exist a short version of the argparse-flag
            and what it should be.
                If set to 'None' no short version will be generated.
                If set to an empty string a short version will be derived from 'name'.
                If set to a string, the string will be used. Cannot start with an '-'
                and cannot contain more characters
                    than 'name'
        """
    ...
    def add_type(
        ...
        Since the type has to be expressable in the TOML-format only 'int' and
        'float' should make sense.
        Args:
            name (): see :py:meth:`ArgConfig.add_arg`
            help_str (): see :py:meth:`ArgConfig.add_arg`
    ...
    def add_narg(
        ...
        Args:
            nargs (): Associate different number of command-line arguments with a
            single action. Mirrors the 'nargs'
