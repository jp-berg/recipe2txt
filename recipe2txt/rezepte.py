...
def get_info(name: str, func: Callable[[Parsed], str | int | float], data: Parsed, context: Context) -> str:
    try:
        info = str(func(data))
...
def parsed2recipe(url: URL, parsed: Parsed, context: Context) -> None:
    ...
    for instruction in extraction_instructions:
        info[instruction[0]] = get_info(*instruction, parsed, context=context)
...
def cutoff_at(url: URL, *identifiers: str) -> URL:
...
if __name__ == "__main__":
    ...
    if 1 == len(sys.argv):
        ...
        if args_are_files:
            unprocessed = read_files(*sys.argv[1:])
        else:
