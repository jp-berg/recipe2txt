...
$(PY_DEPS): # Check if the target is installed, if not try to install it via pip, if that does not work exit
	@(shell command -v $@ 2> /dev/null) || $(PIP) install $@ || (echo "Program '"$@"' not found and cannot be installed via" $(PIP) && exit 1)

$(EXT_DEPS):
	@(shell command -v $@ 2> /dev/null) || (echo "Program '"$@"' not found" && exit 1)
