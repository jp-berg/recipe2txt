...
def install_deps(session: nox.Session, python_path: str = "python") -> None:
    ...
    to_install = [f"{name}{constraint.replace(' ', '')}" for name, constraint in dependencies
                  if not is_available(session, name, python_path)]
    if to_install:
        session.run(python_path, "-m", "pip", "install", *to_install, external=True, silent=True)


def run(session: nox.Session, program: str, *args: str, use_root_env: bool = False) -> None:
...
def is_available(session: nox.Session, package_name: str, python_path: str = "python3",) -> bool:
    out = session.run(python_path, "-c",
                      f"import {package_name.replace('-', '_')}", success_codes=[0, 1], external=True, silent=True)
    return False if out else True


def is_applicable(session: nox.Session, constraints: str) -> bool:
    constraints_list = [[component for component in con.split(" ") if component] for con in constraints.split(";") if con][1:]
...
def dev(session: nox.Session) -> None:
    ...
    session.run(python, "-m", "pip", "install", "nox", external=True, silent=True) #so mypy can typecheck this file
...
def all(session: nox.Session) -> None:
    ...
    session.notify("check")
    install_deps(session)
    session.run("python", "-m", "test.test4recipe2txt", "--delete-database",
                "--long-timeout", "--number-of-urls", "-1", env=root_env)
